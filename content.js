...
\
      ...
      <div class="unm-page" id="unm-page-usercards">\
        ...
        <div class="unm-list unm-settings">\
          ...
          <label><input type="checkbox" id="unm-showMMR" ' + (settings.showMMR ? 'checked' : '') + ' /> Show ELO</label>\
...
// ── ELO Response Cache ────────────────────────────────────────────────────────
// Intercepts the site's own fetch/XHR responses for user/profile API calls,
// caches the raw JSON so we can extract ELO without needing a separate request.
const _playerDataCache = {};   // keyed by normalised username/ID string

// URL patterns that indicate a user profile or stats API response
const _USER_API_PATTERNS = [
  /\/api\/(?:v\d+\/)?users?\//i,
  /\/api\/(?:v\d+\/)?profile\//i,
  /\/api\/(?:v\d+\/)?player\//i,
  /\/api\/(?:v\d+\/)?stats\//i,
...
function _isUserApiUrl(url) {
  if (!url || typeof url !== 'string') return false;
  return _USER_API_PATTERNS.some(p => p.test(url));
}

// Extract a cache key (username or numeric id) from a URL
function _cacheKeyFromUrl(url) {
  const m = url.match(/\/(?:users?|profile|player|stats)\/([^/?#]+)/i);
  return m ? decodeURIComponent(m[1]).toLowerCase() : null;
}

function _storePlayerDataFromResponse(url, data) {
  if (!data || typeof data !== 'object') return;
  const key = _cacheKeyFromUrl(url);
  if (!key) return;
  // Accept the response if it looks like user data
  const root = data.user || data.profile || data.data?.user || data.data?.profile || data;
  if (root && typeof root === 'object' && !Array.isArray(root)) {
    _playerDataCache[key] = root;
    // Also index by username/id fields present in the data
    const uname = (root.username || root.name || root.displayName || '').toLowerCase();
    const uid   = String(root.id || root.userId || root.user_id || '');
    if (uname && uname !== key) _playerDataCache[uname] = root;
    if (uid   && uid   !== key) _playerDataCache[uid]   = root;
  }
}

// Install a passive fetch wrapper that captures user-API responses for ELO caching.
// Runs once at boot, independent of stealth stalking.
(function installEloResponseCapture() {
  const _origFetch = window.fetch;
  window.fetch = function(resource, init) {
    const url = typeof resource === 'string' ? resource
      : (resource instanceof Request ? resource.url : String(resource || ''));
    const promise = _origFetch.apply(this, arguments);
    if (_isUserApiUrl(url)) {
      promise.then(response => {
        if (response && response.ok) {
          response.clone().json().then(data => {
            _storePlayerDataFromResponse(url, data);
          }).catch(() => {});
        }
      }).catch(() => {});
    }
...
async function fetchPlayerStats(username) {
  // Check the ELO response cache first (populated from the site's own API calls)
  const cacheKey = username.toLowerCase();
  if (_playerDataCache[cacheKey]) return _playerDataCache[cacheKey];
...
async function fetchPlayerStatsFromPage(username) {
  try {
    ...
    // Build a merged candidate from ALL dehydrated queries so we don't lose data
    // (e.g. ELO in query[1] while username/matches are in query[0]).
    const queries = pp.dehydratedState?.queries || [];
    const mergedDehydrated = {};
    for (const q of queries) {
      const d = q?.state?.data;
      if (d && typeof d === 'object' && !Array.isArray(d)) {
        const root = d.user || d.profile || d;
        if (root && typeof root === 'object') Object.assign(mergedDehydrated, root);
      }
    }
    ...
    const candidates = [
      ...
      // React Query / TanStack Query dehydrated state — all queries merged
      Object.keys(mergedDehydrated).length ? mergedDehydrated : null,
      // Individual query slots as a final fallback
...
function buildUserCard(data, anchor) {
  ...
  // Helper: extract ELO/MMR from any object, checking many field name variants.
  // Different gaming platforms and backend frameworks use wildly different names
  // for the same concept (e.g. 'elo', 'mmr', 'rating', 'rankPoints', 'skill', …).
  // We check exhaustively so we surface the value regardless of API naming.
  function _pickElo(obj) {
    if (!obj || typeof obj !== 'object') return null;
    return obj.elo            || obj.eloRating      || obj.elo_rating
        || obj.mmr            || obj.mmrRating       || obj.mmr_rating
        || obj.rating         || obj.ratingValue     || obj.rating_value
        || obj.rankPoints     || obj.rank_points     || obj.rankPoint
        || obj.skill          || obj.skillRating     || obj.skill_rating
        || obj.score          || obj.rankScore       || obj.rank_score
        || obj.currentElo     || obj.current_elo     || obj.currentMmr     || obj.current_mmr
        || obj.competitive_elo|| obj.competitiveElo  || obj.competitiveRating
        || obj.glicko         || obj.trueskill       || obj.rankElo        || obj.rank_elo
        || obj.cs_elo         || obj.faceit_elo      || obj.faceitElo
        || obj.points         || obj.rankValue       || obj.rank_value
        || obj.leaderboard_points || obj.leaderboardPoints
        || null;
  }
  ...
  if (gamesArr.length > 0) {
    ...
    if (activeGame) {
      ...
      // Try ELO from the game object itself, then from its nested stats sub-object
      elo     = _pickElo(activeGame) || _pickElo(activeGame.stats) || null;
    ...
    // If ELO wasn't found in the game object, check the top-level user/data objects
    if (elo === null) {
      elo = _pickElo(u?.stats) || _pickElo(data?.stats) || _pickElo(u) || _pickElo(data) || null;
    }
  } else {
    // Direct stats fallback
    const s = u?.stats || data?.stats || {};
    kd      = s.kd      || u?.kd      || data?.kd      || null;
    elo     = _pickElo(s) || _pickElo(u) || _pickElo(data) || null;
  ...
  // K/D and ELO shown in the header right column (matching unm.pwr layout)
  ...
  const headerStatsHtml = (
    (settings.showKD && kdNum !== null && !isNaN(kdNum)
      ...
      : '') +
    (settings.showMMR && elo !== null
      ? '<div class="unm-card-hstat"><span class="unm-stat-label">ELO:</span>&nbsp;<span class="unm-stat-val">~' + escapeHtml(String(elo)) + '</span></div>'
      : '')
  );
